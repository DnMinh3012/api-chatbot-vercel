...
import { CustomerModel, ReservationRequestModel, TableModel, TableTypeModel, FeedbackModel, MenuModel, DishModel,DishTypeModel } from "../model/index";
...
let CancelReservation = async (psid, RtableId) => {
    try {
        if (!psid || !RtableId) {
            return {
                errCode: 1,
        ...
        let response = {
            attachment: {
                ...
                payload: {
                    ...
                    elements: [
                        {
                            title: `Ấn nút dưới đây để Huỷ giờ đặt bàn của bạn`,
                            buttons: [
                                {
                                    type: "web_url",
